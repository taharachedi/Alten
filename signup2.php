...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Assurez-vous que ces clés existent dans $_POST
    if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO consultant_login (nom, email, password) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $name, $email, $hashed_password);

        if ($stmt->execute()) {
            // Inscription réussie
            $message = "Consultant ajouté avec succès. <a href='consultant-login.html'>Cliquez ici pour vous connecter</a>";
            $success = true;
        } else {
        ...
        <div class="col-md-6">
          <div class="card mt-5">
            ...
            <div class="card-footer text-center">
              <a href="consultant-login.html">Déjà un compte ? Se connecter</a>
            </div>
          </div>
        </div>
      </div>
